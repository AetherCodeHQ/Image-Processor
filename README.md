...
> Image-Processor - Open source tool by Qyroxen

`cli` `golang` `image` `processing` `utilities`

## What is it?

**Image Processor** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Image-Processor --path ./target

# With options
./Image-Processor --path ./target --format json --output report.json

# Verbose mode
./Image-Processor --path ./target --verbose
```
...
### Basic scan
```bash
./Image-Processor --path ./my-project
```

### JSON report
```bash
./Image-Processor --path ./my-project --format json --output report.json
```
...
- name: Run Image Processor
  run: ./Image-Processor --path . --format json --output report.json
```
